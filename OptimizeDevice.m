% Multi-wavelength deflector optimization
...
WavelengthRange = OptParm.Input.WavelengthRange;
SpVecGT = OptParm.Input.SpectrumGroundTruth;
FoMWeights = OptParm.Input.FoMWeight;
...
%Compute incident k-vector, sin(theta)
kParallelForward = sind(OptParm.Input.Theta);

% Compute total Fourier orders, this is for the
% accurate RCWA simulation
NFourier = ceil(OptParm.Simulation.Fourier);
...
% this returns the x and y coordinate of the grid points
[xGrid, yGrid, GridScale] = DefineGrid(OptParm.Simulation.Grid, Period, min(WavelengthRange)); 
Nx = length(xGrid); %Number of x grid points
Ny = length(yGrid); %Number of y grid points
% % reduce the structure to 1D:
% yGrid = period(2)/2; Ny = 1;
...
% BVector = ones(size(BVector));

% Generate thresholding parameters for robustness.
% This is especially useful in practical case
% since it can take fabrication consideration. But
% for this illustration case, we don't need it.
...
Figs = [];
% Initializing plot for geometries, if the
% initialization file gives that the geometry is
% output, then it will add a figure attribute to
% the set Figs
...
% Initializing plot for geometries, if the
% initialization file gives that the efficiency is
% output, then it will add a figure attribute to
% the set Figs
...
% this initializes empty matrixs with each index
% denoting the iteration number,  the number of
% robustness (fab) iteration, and polarization.
NWavelength = length(WavelengthRange);
AbsoluteEfficiency = zeros(MaxIterations,NWavelength,NRobustness,NumPol); 
RelativeEfficiency = zeros(MaxIterations,NWavelength,NRobustness,NumPol);

FoM_list = zeros(MaxIterations, 1);
FoM_leastnorm = zeros(MaxIterations, 1);
...
for iter = iterStart:MaxIterations
    ...
    % the first filter is a Guassian lp filter
    FilteredPattern = DensityFilter2D(DevicePattern,BlurGridLarge);
    % the second filter is forcing the
    % binarization, which pushes the value of the
    % pattern to 0 if <0.5, or 1 if >0.5...
    BinaryPattern = ThreshFilter(FilteredPattern,BVector(iter),0.5);
  
    GradientsAll = cell([NRobustness, 1]);
    DispPattern = cell([NRobustness, 1]);
    
    % set a few empty matrices for storing the
    % wavelength-dependent information:

    % perform stochastic selection: first selecting a few entries from the wavelength array
    NWavelength = length(WavelengthRange);
    NPatch = OptParm.Optimization.SGD.NPatch;
    % Randomly select n indices from A without replacement
    % Note: 'Replace', false ensures no index is selected more than once
    selectedIndices = datasample(1:NWavelength, NPatch, 'Replace', false);
    % Sort the selected indices to maintain the original order
    sortedIndices = sort(selectedIndices);
    
    % Use the sorted indices to select entries
    % from wavelength list:
    WavelengthPatch = WavelengthRange(sortedIndices);
    
    % construct empty matrix
    GradientsSpVec = cell([NPatch, 1]);
    % Spectrum Vector 
    SpVec = cell([NPatch, 1]);
    % Begin Wavelength loop:


    for wavelengthIter = 1:NPatch
        Wavelength = WavelengthPatch(wavelengthIter);
        ...
        for robustIter = 1:NRobustness
            ...
            % This is a Gaussian lp filter. 
            FilteredPattern2 = GaussFilter2D(BinaryPattern,BlurGrid);
            % this is the binarization filter which
            % pushes the values to 0 or 1. 
            ...
            for polIter = 1:NumPol  
                ...
                if (Polarizations(polIter)==1) %For TE polarization
                    ...
                    RelativeEfficiency(iter,wavelengthIter,robustIter,polIter) = TransmittedLight.efficiency_TE(TargetIndex)/sum(sum(TransmittedLight.efficiency));
                ...
                elseif (Polarizations(polIter)==-1) %For TM polarization
                    ...
                    RelativeEfficiency(iter,wavelengthIter,robustIter,polIter) = TransmittedLight.efficiency_TM(TargetIndex)/sum(sum(TransmittedLight.efficiency));
                ...
                AbsoluteEfficiency(iter,wavelengthIter,robustIter,polIter) = AbsEff;
    ...
    Gradient((DevicePattern-Gradient)>1) = DevicePattern((DevicePattern-Gradient)>1)-1;
...
% 
%     Gradient = CurrStepSize*Gradient/max(max(abs(Gradient))); % Re-normalize gradient
    GradientsSpVec{wavelengthIter} = Gradient;
    SpVec{wavelengthIter} = mean(mean(AbsoluteEfficiency(iter,wavelengthIter,:,:),4),3);
    end 
    % the end of wavelength loop
    % calculate the total FoM:
    FoM = 0;
    FoM_ln = 0;
    N_tot = Nx*Ny;
    delta = 1/exp(2); % hyperparameter
    xaverage = mean(DevicePattern,'all');
    Varx = 1/N_tot*sum(log(abs(DevicePattern-xaverage)+delta), 'all');
    alpha_Var = 4; %length(WavelengthRange); % hyperparameter
    for wavelengthIter = 1:NPatch %NWavelength
        FoM = FoM+abs(SpVec{wavelengthIter}-SpVecGT(sortedIndices(wavelengthIter))) + alpha_Var*Varx;
        FoM_ln = FoM_ln+abs(SpVec{wavelengthIter}-SpVecGT(sortedIndices(wavelengthIter)));
    end
    FoM_list(iter) = FoM;
    FoM_leastnorm(iter) = FoM_ln;
    rdn_list = [-1,1];

    % Start to integrate the gradient for FOM:
    dFOMdx = zeros(size(Gradient));
    PSNR_const = psnr(DevicePattern,ones(size(DevicePattern))*xaverage);
    for wavelengthIter = 1:NPatch %NWavelength
        dFOMdx = dFOMdx + 2*GradientsSpVec{wavelengthIter}.*(SpVec{wavelengthIter}-SpVecGT(sortedIndices(wavelengthIter))); % + alpha_Var*(1/N_tot*sign(DevicePattern-xaverage+delta/100*randsample(rdn_list,1))./(abs(DevicePattern-xaverage)+delta));
    end
    % 

    % Add gradient to device
    DevicePattern = DevicePattern - NPatch/NWavelength*dFOMdx;
    ...
    ShowProgress(OptParm, {xGrid,yGrid}, DispPattern{RobustInd}, iter, MaxIterations, AbsoluteEfficiency, RelativeEfficiency, Figs, FoM_list, FoM_leastnorm)
    toc;
end
...
OptOut.FoMs = FoM_list;
OptOut.FoMs_LN = FoM_leastnorm;
end
