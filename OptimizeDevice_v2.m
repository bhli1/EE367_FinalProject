...
for iter = iterStart:MaxIterations
    ...
    NWavelength = length(WavelengthRange);
    GradientsSpVec = cell([NWavelength, 1]);
    % Spectrum Vector 
    SpVec = cell([NWavelength, 1]);
    % Begin Wavelength loop:
    for wavelengthIter = 1:NWavelength
        Wavelength = WavelengthRange(wavelengthIter);
        ...
        for robustIter = 1:NRobustness
            ...
            FinalPattern(FinalPattern<0) = 0;                                              
            FinalPattern(FinalPattern>1) = 1;
            ...
            Gradient(FinalPattern>1) = 0; Gradient(FinalPattern<0) = 0; 
...
%     Gradient((DevicePattern-Gradient)>1) = CurrStepSize*DevicePattern((DevicePattern-Gradient)>1)-1;
%     Gradient((DevicePattern-Gradient)<0) = CurrStepSize*DevicePattern((DevicePattern-Gradient)<0);

%     Gradient = Gradient/max(max(abs(Gradient))); % Re-normalize gradient
    ...
    for wavelengthIter = 1:NWavelength
        dFOMdx = dFOMdx + (2*(FoMWeights(wavelengthIter).*GradientsSpVec{wavelengthIter}.*SpVec{wavelengthIter})...
            -2*(FoMWeights(wavelengthIter).*SpVecGT(wavelengthIter).*GradientsSpVec{wavelengthIter}));
    end
    % 

    % Add gradient to device
    DevicePattern = DevicePattern - dFOMdx;
